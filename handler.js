 ...
 *ꨄ Si consideras que esto es un error y cuentas con pruebas, puedes comunicarte con el propietario(a) del bot para apelar la suspensión.*`.trim();
              m.reply(messageText);
          ...
          if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
            if (user.commandCount === 2) {
              ...
              if (remainingTime > 0) {
                const messageText = `*ꨄ Espera* _${remainingTime} segundos_ *antes de utilizar otro comando.*`;
                m.reply(messageText);
                return;
              } else {
                user.commandCount = 0;
        ...
        if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
          mconn.conn.reply(m.chat, `*ꨄ Sus diamantes se han agotado, puede adquirir más con el comando:* _${usedPrefix}buyall_`, m);
          continue; 
        }
        if (plugin.level > _user.level) {
          mconn.conn.reply(m.chat, `*ꨄ Se require tener el nivel ${plugin.level} para poder utilizar el comando. Tú nivel actual es ${_user.level}, usa el comando ${usedPrefix}lvl para subir tu nivel con XP.*`, m);
          continue; 
        }
        const extra = {
          match,
        ...
        try {
          ...
          if (m.limit) {
            m.reply('*ꨄ Se utilizaron ' + +m.limit + ' diamante(s) (limites).*');
          }
        }
        break;
      }
 ...
 *ꨄ Para desactivar la función* _antidelete_*, envia el siguiente comando:* _/disable antidelete_`.trim();
...
global.dfail = (type, m, conn) => {
  const msg = {
    rowner: '*ꨄ Este comando solo puede ser utilizado por el propietario del bot.*',
    owner: '*ꨄ Este comando solo puede ser utilizado por el propietario del bot.*',
    mods: '*ꨄ Este comando solo puede ser utilizado por moderadores y el propietario del bot.*',
    premium: '*ꨄ Este comando solo puede ser utilizado por usurios premium y el propietario del bot.*',
    group: '*ꨄ Este comando solo puede ser utilizado en grupos.*',
    private: '*ꨄ Este comando solo puede ser utilizado en el chat privado del bot.*',
    admin: '*ꨄ Este comando solo puede ser usado por administradores del grupo.*',
    botAdmin: '*ꨄ Para utilizar este comando es necesario que el bot sea administrador del grupo.*',
    unreg: '*ꨄ Para utilizar este comando debes estar registrado.*\n\n*[ 💡 ] Utiliza el comando:* _/verificar nombre.edad_ *para registrarte.*',
    restrict: '*ꨄ Este comando fue desactivado por el propietario del bot.*',
  }[type];
  const aa = {quoted: m, userJid: conn.user.jid};
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '', body: '𝗢𝗽𝘁𝗶𝗺𝘂𝘀 𝗕𝗼𝘁', thumbnail: imagen1, sourceUrl: 'https://www.instagram.com/optimus.ventas.bot'}}}}, aa);
